<x-app-layout>
                    ...
                    @foreach ($estudios as $estudio)
                        <tr class="bg-white">
                            ...
                            <td class="px-6 py-4 flex">
                                <button id="openModalEditBtn" type="button" data-id="{{ $estudio->id }}"
                                ...
                                <form action="{{ route('estudios.destroy', $estudio->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('¿Quieres eliminar este estudio?');"
                                        class="font-medium text-lg text-blue-600 hover:bg-green-100 hover:underline py-1 px-2 border border-blue-500 rounded-lg mr-1"><i
    ...
    <div id="myModalEdit" class="modal hidden fixed w-full h-full top-0 left-0 items-center justify-center z-50">
        <div class="modal-overlay absolute w-full h-full bg-gray-900 opacity-80"></div>
        <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
            <!-- Modal Content -->
            <div class="modal-content py-4 text-left px-6">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" action="" method="POST" id="editarEstudioForm">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="id" id="estudioId">
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            <label for="nombreDelEstudio" class="block mb-2 text-sm font-medium text-gray-900 ">Nombre
                                del estudio</label>
                            <input type="text" name="nombreDelEstudio" id="nombreDelEstudioEdit"
                                class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 ">
                        </div>
                        <div class="col-span-2">
                            <label for="precio" class="block mb-2 text-sm font-medium text-gray-900 ">Precio</label>
                            <input type="text" name="precio" id="precioEdit"
                                class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "
                                placeholder="Escribe el precio del estudio">
                        </div>
                    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                openModalEditBtn.addEventListener('click', function() {
                    var estudioId = this.getAttribute('data-id');

                    fetch(`/obtener-estudio/${estudioId}`)
                        .then(response => response.json())
                        .then(data => {
                            // Llena el formulario de edición con los datos del estudio
                            document.getElementById('estudioId').value = data.id;
                            document.getElementById('nombreDelEstudioEdit').value = data.nombreDelEstudio;
                            document.getElementById('precioEdit').value = data.precio;

                            document.getElementById('editarEstudioForm').action =
                                `/estudios/${data.id}`;
